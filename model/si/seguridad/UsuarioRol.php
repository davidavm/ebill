...
*/
     ...
     */
     ...
     const NONE = 0;
     // Operations
     const INSERT = "INSERT TABLE USUARIO_ROL";
     const UPDATE = "UPDATE TABLE USUARIO_ROL";
     const DELETE = "DELETE TABLE USUARIO_ROL";
     const SELECT = "SELECT TABLE USUARIO_ROL";
     ...
     * @deprecated No.
     */
        public function getList($idUsuarioRol = self::ALL){
            $result = null;
            $query = null;
            try{
                $query = " 	
                ...
                if( $idUsuarioRol != self::ALL){
                $query = $query." and a.pk_id_usuario_rol = ?";
                $result = DataBase::getArrayListQuery($query, array($idUsuarioRol), $this->instanceDataBase);
                }
                else{
     ...
     */
        public function insert($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call usuario_rol_alta(?,?,?,?,?,?,@resultado);  ");
        ...
        public function delete($datos){
            ...
            try{
              $id=-1;                       
                ...
                $sentencia = $gbd->prepare("call usuario_rol_baja(?,?,?,?,@resultado);  ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000);  
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 
     ...
     */
        public function update($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call usuario_rol_modif(?,?,?,?,?,?,@resultado); ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 

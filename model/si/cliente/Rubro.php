...
*/
     ...
     */
     ...
     const NONE = 0;
     // Operations
     const INSERT = "INSERT TABLE RUBRO";
     const UPDATE = "UPDATE TABLE RUBRO";
     const DELETE = "DELETE TABLE RUBRO";
     const SELECT = "SELECT TABLE RUBRO";
     ...
     * @deprecated No.
     */
        public function getList($idRubro = self::ALL, $idEmpresa = self::ALL){
            $result = null;
            $query = null;
            try{
                $query = " 	
                ...
                if( $idRubro != self::ALL){
                    if($idEmpresa != self::ALL){
                        $query = $query." AND pk_id_rubro = ? AND fk_id_empresa = ? ";
                        $result = DataBase::getArrayListQuery($query, array($idRubro, $idEmpresa), $this->instanceDataBase);
                    } else{
     ...
     */
        public function insert($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call rubro_alta(?,?,?,?,?,?,@resultado);  ");
        ...
        public function delete($datos){
            ...
            try{
              $id=-1;                       
                ...
                $sentencia = $gbd->prepare("call rubro_baja(?,?,?,?,@resultado);  ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000);  
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 
     ...
     */
        public function update($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call rubro_modif(?,?,?,?,?,?,@resultado); ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 

...
*/
     ...
     */
     ...
     const NONE = 0;
     // Operations
     const INSERT = "INSERT TABLE ImportacionDatos";
     const UPDATE = "UPDATE TABLE ImportacionDatos";
     const DELETE = "DELETE TABLE ImportacionDatos";
     const SELECT = "SELECT TABLE ImportacionDatos";
     ...
     * @deprecated No.
     */
        public function getList($idImportacionDatos = self::ALL, $idEmpresa = self::ALL){
            $result = null;
            $query = null;
            try{
                $query = " 	
                                  select 
                                   ...
                                   date_format(fecha_transaccion,'%Y-%m-%d %H:%i-%s')  as fecha_transaccion,                                        
                                    ...
                                    fk_id_empresa 
                                from importacion_datos
                                where estado_registro='A'
                                ";

                if( $idImportacionDatos != self::ALL){
                    if($idEmpresa != self::ALL){
                        $query = $query." AND pk_id_importacion = ? AND fk_id_empresa = ? ";
                        $result = DataBase::getArrayListQuery($query, array($idImportacionDatos, $idEmpresa), $this->instanceDataBase);
                    } else{
                        $query = $query." AND pk_id_importacion = ?";
                        $result = DataBase::getArrayListQuery($query, array($idImportacionDatos), $this->instanceDataBase);
                    }                
                } else{
                    if($idEmpresa != self::ALL){
     ...
     */
        public function insert($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call importacion_datos_alta(?,?,?,?,?,?,?,?,?,?,@resultado);  ");
        ...
        public function delete($datos){
            ...
            try{
              $id=-1;                       
                ...
                $sentencia = $gbd->prepare("call importacion_datos_baja(?,?,?,?,@resultado);  ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000);  
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 
     ...
     */
        public function update($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call importacion_datos_modif(?,?,?,?,?,?,?,?,?,?,@resultado); ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 

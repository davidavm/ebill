...
*/
     ...
     */
     ...
     const NONE = 0;
     // Operations
     const INSERT = "INSERT TABLE TIPOCOMPRA";
     const UPDATE = "UPDATE TABLE TIPOCOMPRA";
     const DELETE = "DELETE TABLE TIPOCOMPRA";
     const SELECT = "SELECT TABLE TIPOCOMPRA";
     ...
     * @deprecated No.
     */
        public function getList($idTipoCompra = self::ALL){
            $result = null;
            $query = null;
            try{
                $query = " 	
                ...
                if( $idTipoCompra != self::ALL){
                $query = $query." and a.pk_id_tipo_compra = ?";
                $result = DataBase::getArrayListQuery($query, array($idTipoCompra), $this->instanceDataBase);
                }
                else{
     ...
     */
        public function insert($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call tipo_compra_alta(?,?,?,?,?,?,@resultado);  ");
        ...
        public function delete($datos){
            ...
            try{
              $id=-1;                       
                ...
                $sentencia = $gbd->prepare("call tipo_compra_baja(?,?,?,?,@resultado);  ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000);  
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 
     ...
     */
        public function update($datos){
            ...
            try{
                ...
                $sentencia = $gbd->prepare("call tipo_compra_modif(?,?,?,?,?,?,@resultado); ");
                $sentencia->bindParam(1, $datos[0], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(2, $datos[1], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(3, $datos[2], PDO::PARAM_STR, 4000); 
                $sentencia->bindParam(4, $datos[3], PDO::PARAM_STR, 4000); 
